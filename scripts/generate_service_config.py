...
SERVICE_CONFIGS: Dict[str, Dict[str, Any]] = {
    ...
    "job_manager": {
        ...
        "cpu_limit": "2",
        "replicas": 1,
        "volumes": ["./data/jobs:/app/data/jobs", "./logs/job_manager:/app/logs"],
        "environment": [
    ...
    "control": {
        ...
        "cpu_limit": "1",
        "replicas": 1,
        "volumes": ["./logs/control:/app/logs", "./config/control:/app/config:ro"],
        "environment": ["SYSTEM_MODE=production", "MONITORING_ENABLED=true"],
        "depends_on": ["redis"],
    },
    "embedding_server": {
    ...
    "llm_service": {
        ...
        "cpu_limit": "2",
        "replicas": 1,
        "volumes": ["./logs/llm_service:/app/logs", "./config/llm:/app/config:ro"],
        "environment": [
    ...
    "object_review": {
        ...
        "cpu_limit": "2",
        "replicas": 1,
        "volumes": ["./data/reviews:/app/reviews", "./logs/object_review:/app/logs"],
        "environment": [
            "REVIEW_MODE=manual",
    ...
    "nsfw_detection": {
        ...
        "cpu_limit": "2",
        "replicas": 1,
        "volumes": ["./data/results:/app/results", "./logs/nsfw_detection:/app/logs"],
        "environment": [
            "ENSEMBLE_MODE=true",
    ...
    "llm_summarizer": {
        ...
        "cpu_limit": "2",
        "replicas": 1,
        "volumes": ["./logs/llm_summarizer:/app/logs"],
        "environment": [
            "MULTI_LANGUAGE=true",
            "CONTEXT_AWARE=true",
...
def generate_service_config(service_name: str) -> Dict[str, Any]:
    ...
    service_config = {
        ...
        + environment,
        "depends_on": {dep: {"condition": "service_healthy"} for dep in depends_on},
        "networks": ["ai_network"],
        "deploy": {
            "resources": {
                "limits": {"cpus": cpu_limit, "memory": memory_limit},
                "reservations": {"cpus": reserved_cpu, "memory": reserved_memory},
            }
        },
        "healthcheck": {
