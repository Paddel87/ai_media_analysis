...
class InstanceProvider:
        ...
        self.providers = {
            "vast_ai": {
                ...
                "instance_types": [
                    ...
                    "RTX 3090", "RTX 4090", "A100", "A6000", "V100",
                    # Mid-Range GPUs für CLIP
                    "RTX 3080", "RTX 4080", "A5000", "A4000",
                    # Entry-Level GPUs für CLIP
                    "RTX 3070", "RTX 4070", "A3000",
        ...
        async with aiohttp.ClientSession() as session:
            for provider, config in self.providers.items():
                try:
                    ...
                    logger.log_error(f"Fehler beim Abrufen der Preise von {provider}", error=e)
        return prices

    async def _fetch_vast_ai_prices(self, session: aiohttp.ClientSession) -> Dict[str, Any]:
            ...
            for instance in self.providers["vast_ai"]["instance_types"]:
                ...
                if matching_offers:
                    ...
                    prices[instance] = {
                        ...
                        "spot": {"global": avg_price * 0.7},  # Vast.ai bietet oft Rabatte
                        "specs": self.instance_specs[instance],
                        "reliability_score": self._calculate_vast_reliability(matching_offers)
                    }
        except Exception as e:
    ...
    def _calculate_vast_reliability(self, offers: List[Dict]) -> float:
        ...
        try:
            ...
            logger.log_error("Fehler bei der Berechnung des Vast.ai Zuverlässigkeits-Scores", error=e)
            return 0.0
    ...
    def _extract_aws_price(self, data: Dict, instance: str, price_type: str) -> Dict[str, float]:
        """Extrahiert AWS-Preise aus der API-Antwort."""
        prices = {}
        for region in self.providers["aws"]["regions"]:
            try:
                if price_type == "OnDemand":
                    price = data["terms"]["OnDemand"][f"{instance}.{region}"]["priceDimensions"][0]["pricePerUnit"]["USD"]
                else:
                    price = data["terms"]["Spot"][f"{instance}.{region}"]["priceDimensions"][0]["pricePerUnit"]["USD"]
                prices[region] = float(price)
            except KeyError:
                prices[region] = None
        return prices

    def _calculate_parallel_capacity(self, specs: Dict[str, Any], models: List[str]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.log_error("Fehler bei der Berechnung der parallelen Kapazität", error=e)
            return {"clip": 1, "whisper": 1}

    def _calculate_effective_processing_time(self, 
                                          processing_time: float,
                                          models: List[str],
                                          parallel_capacity: Dict[str, int]) -> float:
        """
        ...
        try:
            ...
            spin_up_time = sum(
                self.model_requirements[model]["spin_up_time"]
                for model in models
            )
            ...
            for model in models:
                if parallel_capacity[model] > 1:
                    ...
                    parallel_boost_reduction = parallel_boost ** (parallel_capacity[model] - 1)
                    
                    parallel_time *= time_reduction * scaling_reduction * parallel_boost_reduction
            
            return spin_up_time + parallel_time
            
        except Exception as e:
            logger.log_error("Fehler bei der Berechnung der effektiven Verarbeitungszeit", error=e)
            return processing_time

    def calculate_price_performance(self, prices: Dict[str, Dict[str, Any]], 
                ...
                for region, on_demand_price in data["on_demand"].items():
                    if on_demand_price:
                        ...
                        result = {
                            ...
                            "instance": instance,
                            "region": region,
                            "on_demand_cost": round(on_demand_price * effective_time / 3600, 4),
                            "spot_cost": round(spot_price * effective_time / 3600, 4) if spot_price else None,
                            "performance_score": specs["compute_units"] / on_demand_price,
                        ...
                        if provider == "vast_ai":
                            result["reliability_score"] = data.get("reliability_score", 0.0)
                            # Berücksichtige Zuverlässigkeit und Performance-Rating
                            result["performance_score"] *= (1 + result["reliability_score"]) * specs["performance_rating"]
...
class InstanceManager:
    ...
    async def get_optimal_instance_config(self, 
                                        processing_time: float,
                                        required_models: List[str],
                                        current_load: float) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.log_error("Fehler bei der Berechnung der optimalen Instanzkonfiguration", error=e)
            raise
    ...
    def _calculate_optimal_instance_count(self,
        ...
        try:
            ...
            if current_load >= self.max_utilization:
                ...
                return min(
                    math.ceil(required_capacity / current_load),
                    self.max_instances
                )
    ...
    def _find_best_instance_config(self,
                                 prices: Dict[str, Dict[str, Any]],
                                 instance_count: int,
                                 required_models: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for provider, instances in prices.items():
                for instance, data in instances.items():
                    ...
                    # Berechne effektive Verarbeitungszeit
                    parallel_capacity = self.instance_provider._calculate_parallel_capacity(
                        specs,
                        required_models
                    )
                    
                    effective_time = self.instance_provider._calculate_effective_processing_time(
                        1.0,  # Basis-Zeit
                        required_models,
                        parallel_capacity
                    )
                    ...
                    for model in required_models:
                        if parallel_capacity[model] > 1:
                            scaling_factor *= (
                                self.instance_provider.model_requirements[model]["parallel_factor"] *
                                self.instance_provider.batch_times[model]["scaling_factor"] *
                                self.instance_provider.batch_times[model]["parallel_boost"]
                            )
            ...
            logger.log_error("Fehler bei der Suche nach der besten Instanzkonfiguration", error=e)
            raise
    ...
    async def _create_vast_instance(self, instance_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                headers = {"Authorization": f"Bearer {self.api_config['vast_ai']['api_key']}"}
                
                # Suche nach passendem Angebot
                search_url = f"{self.api_config['vast_ai']['base_url']}/bundles/"
                async with session.get(search_url, headers=headers) as response:
                    if response.status != 200:
                        raise Exception(f"Fehler beim Abrufen der Angebote: {response.status}")
                    ...
                    if not matching_offers:
                        raise Exception(f"Keine passenden Angebote für {instance_type} gefunden")
                    ...
                    async with session.post(create_url, headers=headers, json=create_data) as response:
                        if response.status != 200:
                            raise Exception(f"Fehler beim Erstellen der Instanz: {response.status}")
    ...
    async def _wait_for_ssh(self, instance_id: str, max_retries: int = 30) -> Dict[str, Any]:
        """
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(destroy_url, headers=headers) as response:
                    if response.status != 200:
                        raise Exception(f"Fehler beim Beenden der Instanz: {response.status}")
                    ...
                    async with session.get(status_url, headers=headers) as response:
                        if response.status == 200:
                            ...
                            if "gpu_utilization" in status_data:
                                instance_data["current_load"] = float(status_data["gpu_utilization"]) / 100.0
...
class RestraintDetector:
    def __init__(self):
        ...
        try:
            ...
            self.categories = [
                ...
                "shibari rope", "jute rope", "hemp rope", "nylon rope", "cotton rope",
                "leather cuffs", "leather collar", "leather harness", "leather straps",
                "metal cuffs", "metal collar", "metal chains", "metal rings",
                "rope bondage", "rope harness", "rope suspension",
                
                # Shibari-Techniken und Knoten
                "shibari pattern", "kinbaku", "rope knot", "rope tie",
                "diamond pattern", "karada", "takate kote", "ebi",
                "tsuri", "suspension", "partial suspension",
                
                # BDSM-Ausrüstung
                "spreader bar", "restraint system", "bondage furniture",
                "suspension frame", "bondage bed", "bondage chair",
                "wrist cuffs", "ankle cuffs", "thigh cuffs",
                "nipple clamps", "clamps", "restraint straps",
                
                # Materialien und Texturen
                "rough rope", "smooth rope", "waxed rope", "treated rope",
                "soft leather", "hard leather", "studded leather",
                "metal chain", "metal ring", "metal hook",
                "synthetic rope", "natural fiber rope",
                
                # Sicherheitsausrüstung
                "safety scissors", "rope cutter", "safety release",
                "emergency release", "quick release", "safety equipment",
                
                # Fesselungsart und Kontext
                "self bondage", "partner bondage", "solo bondage",
                "suspension bondage", "partial suspension", "ground bondage",
                "escape proof", "escape possible", "safety risk",
                
                # Anatomische und physische Faktoren
                "wrist position", "ankle position", "neck position",
                "body tension", "muscle strain", "circulation risk",
                "breathing restriction", "pressure points",
                
                # Verfügbare Hilfsmittel
                "safety tools nearby", "no safety tools", "emergency phone",
                "ice release", "timer release", "magnetic release",
                
                # Emotionale und mentale Faktoren
                "distressed person", "calm person", "panicked person",
                "experienced person", "inexperienced person",
                "safeword available", "no safeword", "communication possible",
                
                # Sicherheitsrisiken
                "high risk", "medium risk", "low risk",
                "unsafe position", "safe position",
                "circulation check", "breathing check",
                "pressure check", "temperature check",
                
                # Befreiungsmöglichkeiten
                "self release possible", "self release impossible",
                "partner release needed", "emergency release needed",
                "time based release", "condition based release",
                
                # Fesselungskontext und Zustimmung
                "consensual bondage", "non-consensual bondage", "forced restraint",
                "voluntary submission", "involuntary submission", "coerced submission",
                "resistance", "struggle", "compliance", "cooperation",
                "active participation", "passive submission", "forced position",
                
                # Körperliche und emotionale Anzeichen
                "physical resistance", "verbal resistance", "emotional distress",
                "fear", "anxiety", "panic", "calm acceptance",
                "muscle tension", "body language", "facial expression",
                "tears", "sweating", "rapid breathing", "hyperventilation",
                
                # Zwangsmittel und Überwältigung
                "weapon present", "threat", "intimidation",
                "physical force", "overpowering", "restraint marks",
                "struggle marks", "defensive wounds", "escape attempts",
                
                # Einvernehmliche Zeichen
                "safeword established", "aftercare available", "trust present",
                "communication", "negotiation", "consent check",
                "comfort check", "boundary respect", "scene negotiation",
                
                # Notfallindikatoren
                "immediate danger", "medical emergency", "psychological crisis",
                "trauma response", "dissociation", "freeze response",
                "fight response", "flight response", "fawn response",
                
                # Alleinlassensituationen
                "abandoned person", "left alone", "unattended person",
                "no supervision", "no assistance nearby", "isolated location",
                "remote area", "locked room", "soundproof room",
                "no escape route", "trapped situation", "time limit exceeded",
                
                # Überwachungs- und Sicherheitsaspekte
                "supervision present", "spotter available", "assistant nearby",
                "monitoring system", "camera present", "baby monitor",
                "intercom system", "emergency button", "panic button",
                
                # Zeitliche Faktoren
                "extended duration", "time limit", "timer present",
                "delayed release", "scheduled check",
                "regular monitoring", "check-in system",
                
                # Umgebungsfaktoren
                "safe environment", "unsafe environment", "hazardous conditions",
                "temperature risk", "ventilation risk", "fire hazard",
                "flood risk", "structural risk", "environmental danger",
                
                # Kommunikationsmöglichkeiten
                "communication device", "phone nearby", "walkie talkie",
                "signal system", "visual signal", "audible signal",
                "emergency contact", "neighbor alert", "community watch"
            ]
            ...
            self.audio_categories = [
                # Notfallgeräusche
                "screaming", "crying", "shouting", "help", "emergency",
                "distress", "panic", "fear", "pain", "struggle",
                "banging", "knocking", "breaking", "crashing",
                
                # Kommunikationsgeräusche
                "talking", "negotiation", "safeword", "consent",
                "check-in", "communication", "discussion",
                
                # Umgebungsgeräusche
                "silence", "background noise", "music", "tv",
                "footsteps", "door", "window", "ventilation",
                
                # Emotionale Geräusche
                "laughing", "moaning", "breathing", "sighing",
                "whimpering", "sobbing", "hyperventilation"
            ]
    ...
    async def analyze_audio(self, audio_data: np.ndarray, sample_rate: int) -> Dict[str, Any]:
        ...
        try:
            ...
            long_silence = any((end - start) / sample_rate > self.min_silence_duration 
            ...
            has_emergency_words = any(word in transcription.lower() for word in emergency_words)
            ...
            emotional_indicators = {
                "distress": any(word in transcription.lower() for word in 
                              ["help", "stop", "pain", "scared", "afraid"]),
                "consent": any(word in transcription.lower() for word in 
                             ["yes", "okay", "continue", "good"]),
    ...
    def _estimate_processing_time(self, frame_count: int, audio_duration: Optional[float] = None) -> float:
        """
        ...
        try:
            ...
            if audio_duration is not None:
                audio_segments = math.ceil(audio_duration / 30)  # 30 Sekunden pro Segment
    ...
    def _calculate_gpu_cost(self, processing_time: float, currency: str = "EUR") -> Dict[str, float]:
        """
    ...
    async def _get_instance_costs(self, processing_time: float) -> Dict[str, Any]:
        ...
        try:
            ...
            price_performance = self.instance_manager.instance_provider.calculate_price_performance(prices, processing_time)
            
            return {
                "available_instances": price_performance,
                "recommended_instance": price_performance[0] if price_performance else None,
    ...
    async def analyze_frame(self, frame: np.ndarray, audio_data: Optional[np.ndarray] = None, 
        ...
        try:
            ...
            for idx, (category, score) in enumerate(zip(self.categories, similarity[0])):
                if score > 0.1:  # Nur relevante Ergebnisse
                    ...
                    if any(indicator in category for indicator in [
                        "consensual", "voluntary", "safeword", "aftercare",
                        "trust", "communication", "negotiation", "cooperation"
                    ]):
                        consent_assessment["consent_indicators"].append(category)
                    elif any(indicator in category for indicator in [
                        "non-consensual", "forced", "resistance", "struggle",
                        "threat", "intimidation", "weapon", "coerced"
                    ]):
                        consent_assessment["non_consent_indicators"].append(category)
                    
                    # Überwachungsindikatoren sammeln
                    if any(indicator in category for indicator in [
                        "supervision", "spotter", "assistant", "monitoring",
                        "camera", "regular", "check-in", "safe environment"
                    ]):
                        supervision_assessment["supervision_indicators"].append(category)
                    elif any(indicator in category for indicator in [
                        "abandoned", "alone", "unattended", "no supervision",
                        "isolated", "remote", "locked", "soundproof"
                    ]):
                        supervision_assessment["abandonment_indicators"].append(category)
            ...
            if audio_analysis:
                if audio_analysis["audio_analysis"]["has_emergency_words"]:
                    consent_assessment["non_consent_indicators"].append("verbal emergency")
                ...
                if audio_analysis["audio_analysis"]["has_long_silence"]:
                    supervision_assessment["abandonment_indicators"].append("extended silence")
                    supervision_assessment["emergency_intervention_needed"] = True
            ...
            # Kostenberechnung
            audio_duration = len(audio_data) / sample_rate if audio_data is not None else None
    ...
    async def process_batch(self, frames: List[np.ndarray], audio_data: Optional[List[np.ndarray]] = None,
                          sample_rates: Optional[List[int]] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for i, frame in enumerate(frames):
                current_batch.append((frame, audio_data[i] if audio_data else None,
            ...
            total_audio_duration = sum(len(audio) / rate for audio, rate in zip(audio_data, sample_rates)) if audio_data else None
            processing_time = self._estimate_processing_time(len(frames), total_audio_duration)
            instance_costs = await self._get_instance_costs(processing_time)
