...
import logging
from data_schema.person_dossier import PersonDossier, FaceInstance
...
class DossierService:
    ...
    def add_face_instance(self, dossier_id: str, face_instance: FaceInstance) -> PersonDossier:
    ...
    def search_dossiers(self, query: str) -> List[PersonDossier]:
        ...
        for dossier in self.dossiers.values():
            if (query in dossier.temporary_id.lower() or
                (dossier.display_name and query in dossier.display_name.lower()) or
                (dossier.notes and query in dossier.notes.lower())):
