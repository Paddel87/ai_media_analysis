...
class FaceReIDService:
    ...
    async def compare_faces(
        self, face1_embedding: Tuple[float, ...], face2_embedding: Tuple[float, ...]
    ) -> float:
        """
    ...
    async def find_matches(
        ...
        try:
            ...
            for i, embedding in enumerate(embeddings):
                ...
                tasks.append(
                    self.compare_faces(target_embedding_tuple, embedding_tuple)
                )
            ...
            for i, similarity in enumerate(similarities):
                if similarity >= threshold:
                    matches.append({"index": i, "similarity": similarity})
...
async def analyze_face(request: ImageAnalysisRequest):
    ...
    try:
        return await face_reid_service.analyze_face(
            request.image_data, request.job_id, request.media_id, request.source_type
        )
    except Exception as e:
...
async def analyze_batch(request: BatchAnalysisRequest):
    ...
    try:
        ...
        chunks = [
            request.images[i : i + request.batch_size]
...
async def compare_faces(request: FaceComparisonRequest):
    ...
    try:
        similarity = await face_reid_service.compare_faces(
            tuple(request.face1_embedding), tuple(request.face2_embedding)
        )
...
async def find_matches(request: FaceMatchRequest):
    ...
    try:
        return await face_reid_service.find_matches(
            request.target_embedding, request.embeddings, request.threshold
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
