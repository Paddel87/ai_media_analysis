...
class VideoAnalysisRequest(BaseModel):
    ...
    batch_size: Optional[int] = Field(
        4,
        description="Anzahl der Frames pro Batch",
        ge=1,
        le=32
    )
    frame_sampling_rate: Optional[int] = Field(
        2,
        description="Jedes n-te Frame wird analysiert",
        ge=1,
        le=10
    )
    priority: Optional[int] = Field(
        1,
        description="Job-Priorität (1-5)",
        ge=1,
        le=5
    )
...
class ImageAnalysisRequest(BaseModel):
    ...
    batch_size: Optional[int] = Field(
        4,
        description="Anzahl der Bilder pro Batch",
        ge=1,
        le=32
    )
    priority: Optional[int] = Field(
...
async def register_job_with_manager(job_id: str, job_type: str, priority: int):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status != 200:
                    ...
                    logger.log_error("Fehler bei der Job-Registrierung", extra={
                        "job_id": job_id,
                        "job_type": job_type,
        ...
        logger.log_error("Unerwarteter Fehler bei der Job-Manager-Kommunikation", error=e)
        raise HTTPException(
...
async def analyze_videos(
    ...
    """
    try:
        logger.log_info("Starte Video-Analyse", extra={
...
async def analyze_images(
    ...
    """
    try:
        logger.log_info("Starte Bild-Analyse", extra={
...
async def health_check():
    ...
    try:
        ...
        health_status = {
            "status": "healthy" if all([pose_health, ocr_health, nsfw_health, job_manager_health]) else "degraded",
            "services": {
