...
class VisionPipeline:
    def __init__(
        ...
        try:
            ...
            logger.log_error(
                "Fehler bei der Initialisierung der Vision Pipeline", error=e
            )
            raise
    ...
    def _cleanup_gpu_memory(self, force: bool = False):
        ...
        if (
            force
            or (self.frame_counter % self.gpu_cleanup_interval == 0)
            or (time_since_last_cleanup > 300)
        ):  # 5 Minuten
            try:
                ...
                logger.log_info(
                    ...
                    extra={
                        ...
                        "memory_freed": before_metrics["memory_usage"]
    ...
    def _adjust_batch_size(self):
        ...
        if memory_usage > self.gpu_memory_threshold:
            ...
            logger.log_info(
                "Batch-Größe erhöht",
                extra={"new_batch_size": self.batch_size, "memory_usage": memory_usage},
            )
    ...
    async def _analyze_frame_internal(self, frame: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            self.redis_client.setex(
                f"frame:{frame_hash}", 3600, pickle.dumps(result)  # 1 Stunde TTL
            )
    ...
    async def process_image(self, image_path: str, job_id: str) -> Dict[str, Any]:
        ...
        try:
            logger.log_info(
                "Starte Bild-Verarbeitung",
                extra={"image_path": image_path, "job_id": job_id},
            )
            ...
            with open(output_path, "w") as f:
                json.dump({"image_path": image_path, "results": result}, f, indent=2)

            logger.log_info(
                "Bild-Verarbeitung abgeschlossen",
                extra={
    ...
    async def _analyze_pose(
        self, session: aiohttp.ClientSession, frame: np.ndarray
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            async with session.post(
                f"{self.pose_service_url}/analyze", json={"image_data": frame_base64}
            ) as response:
    ...
    async def _analyze_ocr(
        ...
        try:
            ...
            # API-Anfrage
            async with session.post(
                f"{self.ocr_service_url}/analyze", json={"image_data": frame_base64}
            ) as response:
                if response.status == 200:
                    return await response.json()
                else:
    ...
    async def _analyze_nsfw(
        ...
        try:
            ...
            # API-Anfrage
            async with session.post(
                f"{self.nsfw_service_url}/analyze", json={"image_data": frame_base64}
            ) as response:
                if response.status == 200:
                    return await response.json()
                else:
...
if __name__ == "__main__":
    ...
    video_paths = ["/app/data/incoming/video1.mp4", "/app/data/incoming/video2.mp4"]
    ...
    image_paths = ["/app/data/incoming/image1.jpg", "/app/data/incoming/image2.jpg"]
