...
class PersonIdentifier:
    ...
    def register_person(self, 
        ...
        try:
            ...
            voice_id = self.voice_id_service.register_voice(voice_audio, voice_sr, {
                "person_id": person_id,
                **metadata
            })
            ...
            if face_image is not None:
                face_id = self.face_id_service.register_face(face_image, {
                    "person_id": person_id,
                    **metadata
                })
    ...
    def identify_person(self,
        ...
        try:
            ...
            if voice_audio is not None and voice_sr is not None:
                voice_matches = self.voice_id_service.identify_voice(voice_audio, voice_sr, threshold)
            ...
            if face_image is not None:
                ...
                for match in face_matches:
                    ...
                    if person_id in self.person_database:
                        ...
                        existing_result = next((r for r in results if r["person_id"] == person_id), None)
                        if existing_result:
            ...
            for result in results:
                similarities = [s for s in [result["voice_similarity"], result["face_similarity"]] if s is not None]
                if similarities:
                    result["combined_similarity"] = sum(similarities) / len(similarities)
                else:
                    result["combined_similarity"] = None
            
            # Nach kombinierter Ähnlichkeit sortieren
            results.sort(key=lambda x: x["combined_similarity"] if x["combined_similarity"] is not None else 0, reverse=True)
...
class VoiceIDService:
    ...
    def register_voice(self, audio: np.ndarray, sr: int, metadata: Dict) -> str:
        ...
        try:
            ...
            self.voice_embeddings = np.vstack([self.voice_embeddings, embedding]) if len(self.voice_embeddings) > 0 else embedding
    ...
    def identify_voice(self, audio: np.ndarray, sr: int, threshold: float = 0.8) -> List[Dict]:
...
class WhisperService:
    def __init__(self):
        ...
        self.noise_types = [
            NoiseType.HVAC, NoiseType.FAN, NoiseType.TRAFFIC,
            NoiseType.MUSIC, NoiseType.CROWD, NoiseType.MACHINE,
            NoiseType.WIND, NoiseType.RAIN, NoiseType.OTHER
        ]
        self.emotion_labels = [
            EmotionType.PLEASURE, EmotionType.PAIN, EmotionType.FEAR,
            EmotionType.ANXIETY, EmotionType.EXCITEMENT, EmotionType.SUBMISSION,
            EmotionType.DOMINANCE, EmotionType.RESISTANCE, EmotionType.COMPLIANCE,
            EmotionType.NEUTRAL
        ]
    ...
    def detect_noise(self, audio: np.ndarray, sr: int) -> Dict[str, float]:
        ...
        try:
            ...
            noise_scores = {
                NoiseType.HVAC: features["low_freq_energy"] * 0.7 + features["spectral_flatness"] * 0.3,
                NoiseType.FAN: features["mid_freq_energy"] * 0.6 + features["spectral_flatness"] * 0.4,
                NoiseType.TRAFFIC: features["low_freq_energy"] * 0.5 + features["high_freq_energy"] * 0.5,
                NoiseType.MUSIC: features["spectral_flatness"] * 0.8 + features["mid_freq_energy"] * 0.2,
                NoiseType.CROWD: features["zero_crossing_rate"] * 0.7 + features["mid_freq_energy"] * 0.3,
                NoiseType.MACHINE: features["low_freq_energy"] * 0.6 + features["spectral_flatness"] * 0.4,
                NoiseType.WIND: features["high_freq_energy"] * 0.8 + features["spectral_flatness"] * 0.2,
                NoiseType.RAIN: features["high_freq_energy"] * 0.7 + features["zero_crossing_rate"] * 0.3,
                NoiseType.OTHER: features["spectral_flatness"] * 0.5 + features["zero_crossing_rate"] * 0.5
            }
    ...
    def load_audio(self, audio_data: bytes) -> Tuple[np.ndarray, int]:
        ...
        try:
            ...
            if max(noise_scores.values()) > 0.3:  # Schwellenwert für Rauschunterdrückung
    ...
    def analyze_context(self, text: str, emotions: Dict[str, float], noise_scores: Dict[str, float]) -> Dict:
        ...
        try:
            ...
            if noise_scores[NoiseType.CROWD] > 0.5:
                context_analysis["nsfw_score"] *= 0.8  # Reduziere NSFW-Score bei Hintergrundgeräuschen
            if noise_scores[NoiseType.MACHINE] > 0.5:
                context_analysis["restraint_score"] *= 0.9  # Reduziere Restraint-Score bei Maschinengeräuschen
    ...
    async def process_batch(self, audio_files: List[bytes], face_images: Optional[List[bytes]] = None, language: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            for i, audio_data in enumerate(audio_files):
                ...
                face_image = face_images[i] if face_images and i < len(face_images) else None
                ...
                # Kontext analysieren
                context = self.analyze_context(transcription["text"], emotions, noise_scores)
    ...
    async def transcribe_audio(self, audio_data: bytes, face_image: Optional[bytes] = None, language: Optional[str] = None) -> Dict:
        """Transkribiert eine Audiodatei mit Emotionsanalyse und Personenidentifikation"""
        try:
            results = await self.process_batch([audio_data], [face_image] if face_image else None, language)
            return results[0]
        except Exception as e:
            logger.error(f"Fehler bei der Transkription: {str(e)}")
            raise

    async def transcribe_batch(self, audio_files: List[bytes], face_images: Optional[List[bytes]] = None, language: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            face_batches = [face_images[i:i + self.batch_size] for i in range(0, len(face_images), self.batch_size)] if face_images else None
            tasks = [self.process_batch(batch, face_batch if face_batches else None, language) 
                    for batch, face_batch in zip(batches, face_batches if face_batches else [None] * len(batches))]
...
async def transcribe_audio(request: TranscriptionRequest):
    """Transkribiert eine Audiodatei mit Emotionsanalyse und Personenidentifikation"""
    try:
        return await whisper_service.transcribe_audio(request.audio_data, request.face_image, request.language)
    except Exception as e:
...
async def transcribe_batch(request: BatchTranscriptionRequest):
    """Transkribiert einen Batch von Audiodateien mit optionalen Gesichtsbildern"""
    try:
        return await whisper_service.transcribe_batch(request.audio_files, request.face_images, request.language)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
