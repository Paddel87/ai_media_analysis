...
class BatchProcessor:
    ...
    async def _update_batch_status(self, status: str, error_message: Optional[str] = None):
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        raise Exception(f"Fehler beim Aktualisieren des Batch-Status: {await response.text()}")
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        raise Exception(f"Fehler beim Aktualisieren des Job-Status: {await response.text()}")
