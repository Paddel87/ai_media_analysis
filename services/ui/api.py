...
async def create_job(
    ...
    try:
        ...
        base_path = (
            f"data/incoming/{'videos' if job_type == 'video' else 'images'}/{job_id}"
        )
...
@app.get("/jobs")
async def get_jobs(job_type: Optional[str] = None, status: Optional[str] = None):
    try:
        jobs = []
...
async def create_batch(job_ids: List[str]):
    try:
        ...
        for job_id in job_ids:
            ...
            if not job_path:
                raise HTTPException(
                    status_code=404, detail=f"Job {job_id} nicht gefunden"
                )
