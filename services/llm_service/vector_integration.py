...
class VectorDBIntegration:
    ...
    def store_embedding(self, text: str, embedding: List[float], metadata: Dict) -> bool:
        """
    ...
    def search_similar(self, embedding: List[float], limit: int = 5, score_threshold: float = 0.7) -> List[Dict]:
        """
    ...
    def batch_store_embeddings(self, texts: List[str], embeddings: List[List[float]], metadata_list: List[Dict]) -> List[bool]:
        """
        ...
        try:
            ...
            # Batch speichern
            response = requests.post(
                f"{self.vector_db_url}/vectors/batch",
    ...
    def search_by_metadata(self, metadata_filter: Dict, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.vector_db_url}/search/metadata",
                json=search_request
            )
    ...
    def update_metadata(self, vector_id: str, new_metadata: Dict) -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"Fehler beim Aktualisieren der Metadaten: {response.text}")
                return False
