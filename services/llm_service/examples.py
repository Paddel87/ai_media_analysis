...
gpt4_context_example = {
    ...
    "context": [
        {
            "role": "system",
            "content": "Du bist ein Experte für Textzusammenfassung."
        },
        {
            "role": "user",
            "content": "Hier ist der Text zum Zusammenfassen: [Text]"
        }
    ]
}
...
def search_similar_texts(text: str, limit: int = 5, score_threshold: float = 0.7, filter: Dict = None) -> List[Dict]:
    """
...
def main():
    ...
    batch_response = requests.post(
        f"{SERVICE_URL}/batch-embed",
        json=vector_batch_example
    )
    ...
    metadata_search_response = requests.post(
        f"{SERVICE_URL}/search-metadata",
        json=vector_metadata_search_example
    )
    ...
    metadata_update_response = requests.patch(
        f"{SERVICE_URL}/update-metadata",
        json=vector_metadata_update_example
    )
    ...
    index_response = requests.post(
        f"{SERVICE_URL}/create-index",
        json=vector_index_example
    )
