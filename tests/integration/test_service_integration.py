...
class TestVisionVectorDBIntegration:
    ...
    def test_embedding_storage_and_retrieval(
        self, mock_get, mock_post, sample_image_data
    ):
...
class TestLLMVectorDBIntegration:
    ...
    def test_text_embedding_and_storage(self, mock_post, sample_text_data):
        ...
        response = requests.post(
            "http://llm-service:8001/embed_texts", json={"texts": sample_text_data}
        )
                        ...
                        "segments": [
                            {
                                "start": 0.0,
                                "end": 3.5,
                                "text": "Dies ist eine gesprochene Nachricht",
                            },
...
class TestFullPipelineIntegration:
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_complete_media_analysis_pipeline(
        self, mock_get, mock_post, sample_image_data
    ):
        ...
        mock_post.side_effect = [
            ...
            Mock(
                status_code=201,
                json=Mock(return_value={"stored": True, "image_id": "img_123"}),
            ),
            # LLM Analysis
            Mock(
                status_code=200,
        ...
        vision_response = requests.post(
            "http://vision-pipeline:8000/analyze", json={"image_data": "base64_image"}
        )
        ...
        storage_response = requests.post(
            ...
            json={
                "embedding": vision_results["embedding"],
        ...
        llm_response = requests.post(
            "http://llm-service:8001/analyze", json={"vision_results": vision_results}
        )
...
class TestServiceHealthAndResilience:
    ...
    def test_service_retry_mechanism(self, mock_post):
        ...
        mock_post.side_effect = [
            ...
            Mock(
                status_code=200, json=Mock(return_value={"result": "success"})
            ),  # Dritte erfolgreich
        ]
    ...
    def test_service_timeout_handling(self, mock_post):
        ...
        def slow_response(*args, **kwargs):
            ...
            return Mock(
                status_code=200, json=Mock(return_value={"result": "slow_success"})
            )
