...
class TestBaseService:
    ...
    def test_config_validation_invalid(self, mock_config):
        ...
        service = MockBaseService(mock_config)
        invalid_config = {
            key: value for key, value in mock_config.items() if key != "api_host"
        }
...
class TestServiceCommunication:
    ...
    def test_http_get_request(self, mock_get):
        ...
        mock_get.return_value = Mock(
            status_code=200, json=Mock(return_value={"data": "test"})
        )
    ...
    def test_http_post_request(self, mock_post):
        ...
        mock_post.return_value = Mock(
            status_code=201, json=Mock(return_value={"id": "123", "status": "created"})
        )
        ...
        mock_post.assert_called_once_with(
            "http://test-service/data", json={"test": "data"}
        )
